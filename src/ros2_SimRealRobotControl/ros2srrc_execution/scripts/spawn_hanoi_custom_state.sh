...
# Hanoi-style defaults: table center (0, 0.48, 0.42); top surface z=0.84 m
TABLE_X=0.0
TABLE_Y=0.48
TABLE_Z=0.42
PEG_Y=0.58
PEG0_X=-0.2
PEG1_X=0.0
TABLE_SURFACE_Z=0.84   # table_z + table_height/2 = 0.42 + 0.42

echo "Spawning table..."
python3 "$SPAWN" --package ros2srrc_objects --urdf box.urdf.xacro --name table1 \
  --x $TABLE_X --y $TABLE_Y --z $TABLE_Z \
  --size_x 1.0 --size_y 0.8 --size_z 0.84 --mass 50.0 --color white

sleep 0.2

# Peg 0: 1 cube same size as Hanoi cube_0 (0.09 m)
SIZE_CUBE0=0.09
Z_PEG0_CUBE=$(python3 -c "print(0.84 + 0.09/2)")
