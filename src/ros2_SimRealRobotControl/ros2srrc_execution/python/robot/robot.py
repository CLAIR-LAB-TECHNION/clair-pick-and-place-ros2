...
class RBT():
    ...
    def Move_EXECUTE(self, ACTION):
        ...
        # Timeout for action execution (30 seconds)
        TIMEOUT = 30.0
        
        self.MoveClient.send_goal(ACTION)
        while rclpy.ok():
            rclpy.spin_once(self.MoveClient, timeout_sec=0.1)
            ...
            if elapsed > TIMEOUT:
                RES["Message"] = "MoveG:TIMEOUT"
                RES["Success"] = False
                print(f'[CLIENT - robot.py]: Move ACTION TIMEOUT after {TIMEOUT}s')
                break
    ...
    def RobMove_EXECUTE(self, TYPE, SPEED, POSE):
        ...
        # Timeout for action execution (60 seconds for robot movements)
        TIMEOUT = 60.0

        self.RobMoveClient.send_goal(TYPE, SPEED, POSE)
        while rclpy.ok():
            rclpy.spin_once(self.RobMoveClient, timeout_sec=0.1)
            ...
            elapsed = time.time() - T_start
            if elapsed > TIMEOUT:
                RES["Message"] = "RobMove:TIMEOUT"
                RES["Success"] = False
                print(f'[CLIENT - robot.py]: RobMove ACTION TIMEOUT after {TIMEOUT}s')
                break
