...
"""
pick_auto.py - Automated Pick Action
...
Usage:
    ros2 run ros2srrc_execution pick_auto.py object:=cube1
...
# Import the Pick class from pick.py
...
class ObjectPoseGetter(Node):
    ...
    def pose_callback(self, msg):
        """Callback when object pose is received"""
        if not self.pose_received:
            ...
            self.get_logger().info(f'Received pose for {self.object_name}: x={self.object_pose.x:.3f}, y={self.object_pose.y:.3f}, z={self.object_pose.z:.3f}')
...
def main(args=None):
    ...
    if object_name is None:
        ...
        print("")
        print("Usage: ros2 run ros2srrc_execution pick_auto.py object:=<object_name>")
        print("")
        print("Example:")
        print("  ros2 run ros2srrc_execution pick_auto.py object:=cube1")
        print("")
    ...
    if cube_size_arg:
        ...
        print(f"[Pick Auto]: Using default gripper close percentage: {gripper_value:.2f}%")
    ...
    # Wait for pose
    print(f"[Pick Auto]: Waiting for object pose from /object_poses/{object_name}...")
    ...
    print(f"[Pick Auto]: Object pose retrieved successfully!")
    ...
    class PoseChecker(Node):
        def __init__(self):
            super().__init__('pick_auto_pose_checker')
            self.pose_received = False
    ...
    print("[Pick Auto]: Checking robot pose topic availability...")
    ...
    if pose_checker.pose_received:
        print("[Pick Auto]: Robot pose topic is publishing.")
    else:
        print("[Pick Auto]: Robot pose topic not detected (may not be required).")
    
    # Additional wait to ensure MoveIt!2 planning scene is ready
    print("[Pick Auto]: Waiting for MoveIt!2 planning scene to initialize...")
    time.sleep(2.0)  # Give MoveIt!2 time to fully initialize planning scene
    print("[Pick Auto]: System ready!")
    print("")
    ...
    if min_lift_height is not None:
        print(f"[Pick Auto]: Using minimum lift height: {min_lift_height:.3f}m (to clear obstacles above object)")
        print("")
    
    if fallback_enabled:
        print("[Pick Auto]: Fallback mechanism ENABLED")
        ...
        print("[Pick Auto]: Fallback mechanism DISABLED - will only try single attempt")
        print("")
